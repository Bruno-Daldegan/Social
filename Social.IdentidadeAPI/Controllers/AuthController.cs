...
using Social.IdentidadeAPI.Extensions;
using Social.IdentidadeAPI.Models;
using System;
...
namespace Social.IdentidadeAPI.Controllers
{
    ...
    public class AuthController : MainController
    {
        ...
        public async Task<ActionResult> Login(UsuarioLogin usuarioLogin)
        {
            ...
            if (result.Succeeded)
            {
                return CustomResponse(await GeraJwt(usuarioLogin.Email));
            }

            if (result.IsLockedOut)
            {
                AdicionarErroProcessamento("Usuário bloqueado temporareamente por tentativas inválidas.");
                return CustomResponse();
            }
        ...
        private async Task<UsuarioRespostaLogin> GeraJwt(string email)
        {
        ...
        private async Task<ClaimsIdentity> ObterClaimsUsuario(IdentityUser user, ICollection<Claim> claims)
        {
            ...
            {
                claims.Add(new Claim("role", useRoles));
            }
        ...
        {
            return (long)Math.Round((date.ToUniversalTime() - new DateTimeOffset(1970,1,1,0,0,0, TimeSpan.Zero)).TotalMilliseconds);
        }
    }
}
